    ...
    public class Tool
    {
        ...
        public static ClientInfo Client
        {
            get
            {
                if (client == null)
                {
                    client = new ClientInfo() { GID = Guid.NewGuid(), ClientName = "", StartTime = DateTime.Now, ComputerName = System.Net.Dns.GetHostName(), IP = GetInternalIP(), MAC = GetMacAddress() };
                }
                return client;
            }
        }
