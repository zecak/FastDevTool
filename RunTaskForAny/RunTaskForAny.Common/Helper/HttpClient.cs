...
namespace RunTaskForAny.Common.Helper
{
    public class HttpClient
    {
        ...
        private string userAgent = "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.2; SV1; .NET CLR 1.1.4322; .NET CLR 2.0.50727; .NET CLR 4.0.30319)";
        ...
        private HttpWebRequest CreateRequest()
        {
            ...
            if (verb == HttpVerb.POST)
            {
                ...
                if (files.Count > 0)
                {
                    ...
                    foreach (var key in postingData.Keys)
                    {
        ...
        public byte[] GetBytes()
        {
            ...
            if (rs == null) return memoryStream.ToArray();
        ...
        private string GetEncodingFromBody(byte[] data)
        {
            ...
            var i = dataAsAscii.IndexOf("charset=", StringComparison.Ordinal);
            if (i == -1) return null;
            var j = dataAsAscii.IndexOf("\"", i, StringComparison.Ordinal);
            if (j == -1) return null;
            var k = i + 8;
            string encodingName = dataAsAscii.Substring(k, (j - k) + 1);
            var chArray = new[] { '>', '"' };
